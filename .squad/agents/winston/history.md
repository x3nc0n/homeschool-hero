...
- 2026-07-22T18:07:07.184-05:00 — **Issue #411 Test Matrix — Comprehensive Coverage for Amended API Token Design.** Prepared comprehensive test matrix for #411 following Egon's security amendment. **Test Spec:** 30-35 tests across three categories: (1) Issuance (8 tests: RBAC owner-only, capability validation, duplicate name 409, active limit 409, expires_in_days range 422, non-delegatable cap 400, unique constraint, max-10 enforcement). (2) Bearer verification & revocation (5 tests: decode + jti lookup, registered jti required for token_type=api_token, unregistered/revoked/expired → 401, external JWTs skip table lookup, last_used_at update). (3) Capability intersection (4 tests: role-derived caps computed, intersection applied for api_token type, external JWTs bypass intersection, token cannot exceed issued scope). (4) Cross-family rejection (3 tests: family_id embedded in JWT, FamilyMembership validation, GET/DELETE wrong family → 404). (5) HTTP status codes (10+ coverage: all scenarios from amendment table). **Files:** `backend/tests/test_api_tokens.py` (unit), extend `backend/tests/test_auth_external.py` (bearer/revocation), extend `backend/tests/test_curriculum_ai_import.py` (integration). **Key decision:** Skip concurrent-race tests (flaky in CI); unique constraint is correctness guarantee; count check is UX. **Blockers resolved:** All 5 design gaps now cleared by amendment; test matrix can proceed.
