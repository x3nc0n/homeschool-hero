...
- 2026-07-22T18:42:40.909-05:00 — Issue #411 implementation ships a family-scoped `api_tokens` table with `UNIQUE(family_id, name)`, SHA-256 token digests, and `POST/GET/DELETE /api/auth/api-tokens` guarded by `manage_security`; bearer auth now enforces `token_type=api_token` registration (`jti`, revoked/expired/digest checks => 401), ignores forged `X-Family-Id` overrides for API tokens, and intersects delegated token capabilities with role-derived capabilities in `_resolve_bearer_session_claims` so headless curriculum, submissions, and grading flows work without granting `manage_security`.
