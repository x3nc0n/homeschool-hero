...
- 2026-05-18T17:23:01.387-05:00 — Patched `Dockerfile` so the runtime stage runs `apt-get upgrade -y` before installing runtime packages, which brought `libcap2`, `libsystemd0`, and `libudev1` to fixed Debian 13 versions; also copied `frontend/.npmrc` into the frontend builder before `npm ci` so the container build can honor the repo’s `legacy-peer-deps=true` setting during `docker build --no-cache --target runtime .`, and verified backend tests from a local `.venv` with Python 3.13.
