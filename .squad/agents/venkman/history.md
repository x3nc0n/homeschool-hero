...
- 2026-07-10T21:01:47Z — **FileUpload.tsx state:** After PR #298→main reconciliation, `frontend/src/components/features/FileUpload.tsx` now carries three features simultaneously: (1) canvas/`createImageBitmap` image-preview security refactor (no object URLs, no XSS), (2) sequential step-lock (`previewSelectionId`, `readyForFile` gate, step 1→2→3 indicators), and (3) Android-compatible sr-only ref-click file/camera inputs. `Input` (shadcn) import is removed; only native `<input>` elements are used for file picking.

- 2026-07-10T21:01:47Z — **PR #298→main reconciliation:** PR #305 (UI overhaul with step-lock + canvas security) merged first, creating a conflict for PR #298 (Android upload fix). Resolution approach: rebase #298 onto main, take main's full step-lock structure, apply the Android fix (sr-only inputs + onClick refs) inside the `readyForFile` else-branch, preserve all i18n `t()` calls and `aria-hidden` attributes from main. Build + lint clean after resolution.
