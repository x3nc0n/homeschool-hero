...
const GRADING_STATUS_KEYS: Record<string, string> = {
  pending: 'upload.gradingStatus.pending',
  ocr_processing: 'upload.gradingStatus.ocr_processing',
  ocr_complete: 'upload.gradingStatus.ocr_complete',
  ai_grading: 'upload.gradingStatus.ai_grading',
  ai_complete: 'upload.gradingStatus.ai_complete',
  review_needed: 'upload.gradingStatus.review_needed',
  reviewed: 'upload.gradingStatus.reviewed',
  final: 'upload.gradingStatus.final',
}

function gradingStatusKey(status?: string | null): string {
  if (!status) return 'upload.gradingStatus.pending'
  return GRADING_STATUS_KEYS[status] ?? 'upload.gradingStatus.unknown'
}
...
function formatVersionMeta(version: SubmissionVersion) {
  const details = [formatBytes(version.file_size_bytes)].filter(Boolean)
  ...
  return (
    <PullToRefresh onRefresh={load}>
      <div className="space-y-6">
        ...
        {!capabilities.ai_grading.enabled || !capabilities.ocr.enabled ? (
          <div className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            {!capabilities.ocr.enabled
              ? t('upload.capabilityWarningOcr')
              : t('upload.capabilityWarningAi')}
          </div>
        ) : null}
        ...
        {/* Secondary section: submission management */}
        <div>
          <h2 className="mb-3 text-base font-semibold text-foreground">{t('upload.sectionSubmissions')}</h2>
          <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            ...
            <Card>
              <CardContent className="space-y-3 pt-4">
                {submissions.length ? (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <Badge variant="secondary">{t('upload.currentVersion', { version: submission.submission_version || 1 })}</Badge>
                          <Badge variant="outline">{t(gradingStatusKey(submission.grading_job?.status))}</Badge>
                        </div>
                      </div>
                  ...
                  <p className="py-4 text-center text-sm text-muted-foreground">{t('upload.submissionsEmpty')}</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">{t('upload.sectionDetail')}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {detailLoading ? (
                  ...
                  <>
                    <div className="rounded-lg border p-4 text-sm">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <Badge>{t('upload.currentVersion', { version: selectedSubmission.submission_version || 1 })}</Badge>
                      </div>
                    ...
                    <div className="rounded-lg border p-4 text-sm">
                      <div className="flex items-center justify-between gap-2">
                        <h3 className="font-semibold">{t('upload.gradingProgress')}</h3>
                        <Badge variant={selectedSubmission.grading_job?.status === 'final' ? 'default' : 'secondary'}>
                          {t(gradingStatusKey(selectedSubmission.grading_job?.status))}
                        </Badge>
                      </div>
                      <div className="mt-3 space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {selectedSubmission.grading_job?.status_history?.[Math.max((selectedSubmission.grading_job?.status_history?.length || 1) - 1, 0)]?.detail ||
                            t('upload.gradingStatus.pending')}
                        </p>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between gap-2">
                        <h3 className="text-sm font-semibold">{t('upload.versionHistory')}</h3>
                        <span className="text-xs text-muted-foreground">
                          {t('upload.versionCount', { count: selectedSubmission.version_history.length })}
                        </span>
                      </div>
                      <div className="space-y-3">
                          ...
                          <div key={version.id} className="rounded-lg border p-3 text-sm">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <div className="flex items-center gap-2">
                                <span className="font-medium">{t('upload.currentVersion', { version: version.submission_version || 1 })}</span>
                                {version.is_current ? (
                                  <Badge>{t('upload.versionCurrent')}</Badge>
                                ) : (
                                  <Badge variant="secondary">{t('upload.versionArchived')}</Badge>
                                )}
                              </div>
                              <span className="text-xs text-muted-foreground">
                                {version.uploaded_at ? new Date(version.uploaded_at).toLocaleString() : t('upload.unknownUploadTime')}
                              </span>
                            </div>
                  ...
                  <p className="text-sm text-muted-foreground">{t('upload.latestUpload', { id: latestSubmission.id })}</p>
                ) : (
                  <p className="text-sm text-muted-foreground">{t('upload.detailEmpty')}</p>
                )}
              </CardContent>
            </Card>
          </div>
